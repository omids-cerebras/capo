...
def get_tool_call_parser_type(tokenizer: PreTrainedTokenizer) -> str:
    ...
    for parser_type, parser_cls in items:
        parser = parser_cls()
        if parser.bot_token.strip() in tokenizer.get_vocab() and (
            parser.eot_token == "" or parser.eot_token.strip() in tokenizer.get_vocab()
        ):
            return parser_type
    else:
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        load_format = (
            "dummy" if self.config.load_format.startswith("dummy") else self.config.load_format
        )
    ...
    def update_sampling_params(self, **kwargs):
        ...
        old_sampling_params_args = {
            key: self.sampling_params[key] for key in kwargs if key in self.sampling_params
        }
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    _req.add_tool_response_messages(
                        self.tokenizer, [resp for resp, _, _ in tool_call_results]
                    )
                    for tool_call, (resp, reward, metrics) in zip(
                        parsed_tool_calls, tool_call_results, strict=False
                    ):
                ...
                if (
                    len(_req.get_generation_prompt_ids(self.tokenizer)) + 1
    ...
    async def _handle_engine_call(
        ...
        **kwargs,
        ...
        if "n" not in kwargs or (
            kwargs["n"] > 1 and override_n
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            prompt_attention_mask.append(
                torch.tensor(req.prompt_attention_mask, dtype=torch.int, device=tgt_device)
            )
            ...
            prompt_position_ids.append(
                torch.tensor(req.prompt_position_ids, dtype=torch.int, device=tgt_device)
            )
            response_position_ids.append(
                torch.tensor(req.response_position_ids, dtype=torch.int, device=tgt_device)
            )
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    _tool_schemas = [
                        self._tool_map[k].get_openai_tool_schema() for k in _tools_kwargs.keys()
                    ]
                    _input_ids = None
