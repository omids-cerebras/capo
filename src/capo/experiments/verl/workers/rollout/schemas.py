...
class AsyncRolloutRequest(BaseModel):
    ...
    def _update_input_ids(
        self, new_input_ids: list[int], attention_mask: bool, loss_mask: bool
    ) -> None:
        """
        ...
        self.position_ids += (
            compute_position_id_with_mask(torch.tensor(attention_mask))
            + (self.position_ids[-1] + 1)
        ).tolist()
