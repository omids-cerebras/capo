...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def get_rng_state(
        self, use_dist_ckpt: bool = False, data_parallel_random_init: bool = False
    ):
    ...
    def load_optimizer(self, ckpt_path):
        ...
        log_with_rank(
            f"Loading optimizer from {optimizer_path}", rank=self.rank, logger=logger
        )
    ...
    def load_rng_states(
        ...
        rng_state_path = get_rng_states_checkpoint_path(
            ckpt_path, only_rank0_save=False
        )
        log_with_rank(
            f"Loading rng states from {rng_state_path}", rank=self.rank, logger=logger
        )
        rng_state = torch.load(rng_state_path, weights_only=False)
        # access rng_state for data parallel rank
        if not use_dist_ckpt:
            rng_state = (
                rng_state[mpu.get_data_parallel_rank()]
    ...
    def load_checkpoint(
        ...
        if self.should_load_model:
            ...
            for vpp_rank, (state_dict, model) in enumerate(
                zip(state_dicts, self.model, strict=False)
            ):
                model.load_state_dict(state_dict)
            log_with_rank(
                f"Loaded sharded model checkpoint from {model_path}",
                rank=self.rank,
                logger=logger,
            )
        ...
        if self.should_load_extra:
            ...
            if self.use_checkpoint_opt_param_scheduler:
                optimizer_scheduler_path = get_optimizer_scheduler_checkpoint_path(
                    local_path, only_rank0_save=True
                )
                if os.path.exists(optimizer_scheduler_path):
                    log_with_rank(
                        f"Loading optimizer scheduler from {optimizer_scheduler_path}",
                        rank=self.rank,
                        logger=logger,
                    )
                    state_dict = torch.load(
                        optimizer_scheduler_path, weights_only=False
                    )
                    if self.lr_scheduler is not None:
                        self.lr_scheduler.load_state_dict(state_dict)
                else:
                    log_with_rank(
                        f"Optimizer scheduler path {optimizer_scheduler_path} does not exist, skipping loading.",
                        rank=self.rank,
                        logger=logger,
                    )
    ...
    def save_checkpoint(
        ...
        if self.should_save_model and mpu.get_data_parallel_rank() == 0:
            ...
            log_with_rank(
                f"Saving sharded model checkpoint to {local_path}",
                rank=self.rank,
                logger=logger,
            )
            model_ckpt_path = get_model_checkpoint_path(local_path)
            hf_config_and_tokenizer_path = get_hf_config_and_tokenizer_checkpoint_path(
                local_path
            )
            ...
            log_with_rank(
                f"Saved checkpoint to {model_ckpt_path}", rank=self.rank, logger=logger
            )
            if self.rank == 0:
                ...
                if hdfs_path is not None:
                    ...
                    hdfs_io.copy(
                        src=hf_config_and_tokenizer_path,
                        dst=hdfs_path,
                        dirs_exist_ok=True,
                    )
        ...
        if self.should_save_optimizer:
            ...
            log_with_rank(
                f"Saved optimizer state to {optimizer_path}",
                rank=self.rank,
                logger=logger,
            )
        ...
        if self.should_save_extra:
            ...
            rng_state_path = get_rng_states_checkpoint_path(
                local_path, only_rank0_save=False
            )
            rng_state = self.get_rng_state()
            torch.save(rng_state, rng_state_path)
            log_with_rank(
                f"Saved rng states to {rng_state_path}", rank=self.rank, logger=logger
            )
            ...
            if self.rank == 0:
                ...
                if self.lr_scheduler is not None:
                    ...
                    log_with_rank(
                        f"Saved optimizer scheduler state to {optimizer_scheduler_path}",
                        rank=self.rank,
                        logger=logger,
                        log_only_rank_0=True,
                    )
