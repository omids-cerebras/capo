...
def convert_checkpoint_from_transformers_to_megatron_dpskv3(
    ...
    numel: int = 0
    numel += safe_copy(
        model.embedding.word_embeddings.weight, hf_model.model.embed_tokens.weight
    )
    print(f"{numel=}")
    for layer_idx, (layer, hf_layer) in enumerate(
        zip(model.decoder.layers, hf_model.model.layers)
    ):
        numel_cur: int = numel
        numel += safe_copy(
            layer.input_layernorm.weight, hf_layer.input_layernorm.weight
        )
        ...
        numel += safe_copy(
            layer.self_attention.linear_proj.weight, hf_layer.self_attn.o_proj.weight
        )
        ...
        if not hasattr(layer.mlp, "router"):
            ...
            numel += safe_copy(
                layer.mlp.linear_fc2.weight, hf_layer.mlp.down_proj.weight
            )
        else:
            ...
            if tfconfig.moe_grouped_gemm:
                ...
                for i, hf_expert in enumerate(hf_layer.mlp.experts):
                    ...
                    numel += safe_copy(
                        expert.linear_fc2.weight, hf_expert.down_proj.weight
                    )
            numel += safe_copy(
                layer.pre_mlp_layernorm.weight, hf_layer.post_attention_layernorm.weight
            )
            shared_fc1_weight = torch.cat(
                [
            ...
            numel += safe_copy(
                layer.mlp.shared_experts.linear_fc1.weight, shared_fc1_weight
            )
            numel += safe_copy(
