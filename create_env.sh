...
#   - Create/reuse VENV_DIR.
#   - Install uv inside that env.
#   - Use uv to install pinned deps and CAPO (-e .).
#
# Usage:
...
if [ "${PYTHON_BIN-}" != "" ]; then
  # User explicitly set PYTHON_BIN: respect it, don't fall back silently.
...
# ---------------------------------------------------------------------
# Step 5: Install pinned deps and CAPO
# ---------------------------------------------------------------------

echo
echo "Installing pinned dependencies from '$PINNED_FILE' into $VENV_DIR using uv..."
"$UV_BIN" pip install -r "$PINNED_FILE"

echo
echo "Installing CAPO in editable mode (pip-style -e .) using uv..."
