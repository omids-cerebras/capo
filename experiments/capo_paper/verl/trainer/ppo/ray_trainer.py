...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        # Optional: local line-delimited JSON metrics log (for reproducible paper artifacts).
        # If `trainer.local_metrics_path` is set, we append every `logger.log(...)` payload to that file.
        local_metrics_path = self.config.trainer.get("local_metrics_path", None)
        _local_fh = None
        if local_metrics_path:
            import json, time
            from pathlib import Path

            _p = Path(local_metrics_path)
            _p.parent.mkdir(parents=True, exist_ok=True)
            _local_fh = _p.open("a", encoding="utf-8")

            def _log_local(d: dict, step: int):
                rec = {"step": int(step), "time": float(time.time())}
                for k, v in d.items():
                    # Keep numbers as floats; stringify other values for safety.
                    if isinstance(v, (int, float)):
                        rec[k] = float(v)
                    else:
                        rec[k] = str(v)
                _local_fh.write(json.dumps(rec) + "\n")
                _local_fh.flush()
        ...
        def _close_local_metrics():
            """Close local metrics file handle if enabled."""
            nonlocal _local_fh
        ...
        if self.val_reward_fn is not None and self.config.trainer.get(
            ...
            _log_local(val_metrics, self.global_steps)
            if self.config.trainer.get("val_only", False):
                _close_local_metrics()
                return
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        assert (
                            sum(
                                [
                                    self.config.algorithm.use_dr_grpo,
                                    self.config.algorithm.use_grpopp,
                                ]
                            )
                            <= 1
                        )
                        assert (
                            self.config.algorithm.use_dr_grpo
                            == self.config.actor_rollout_ref.actor.use_dr_grpo
                        ), "must be identical"
                        assert (
                            self.config.algorithm.use_grpopp
                            == self.config.actor_rollout_ref.actor.use_grpopp
                        ), "must be identical"
                        assert dict(self.config.algorithm.grpopp_config) == dict(
                            self.config.actor_rollout_ref.actor.grpopp_config
                        ), "must be identical"
                ...
                _log_local(metrics, self.global_steps)
