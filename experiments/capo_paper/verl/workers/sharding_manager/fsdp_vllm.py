...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        if "vllm_v_0_6_3" in str(type(self.inference_engine)) or "vllm_v_0_5_4" in str(
            type(self.inference_engine)
        ):
            # vLLM <= v0.6.3
            self.model_runner = (
                self.inference_engine.llm_engine.model_executor.worker.model_runner
