...
def main():
    ...
    for r in runs:
        ...
        _ensure(v is not None, f"Run {r.get('run_dir')} missing acc/reward metric; update ACC_KEYS/REWARD_KEYS.")
        main_rows.append((r.get('exp_name','?'), r.get('adv_estimator','?'), float(v)))
    ...
    (args.out/'tab_main_accuracy.tex').write_text(
        _latex_table(table_rows, cols, header=None,
    ...
    for name in ['fig_dynamics.pdf','fig_stability.pdf','fig_length_deciles.pdf']:
        plt.figure(); plt.text(0.5,0.5,f"{name} TBD: populate from metrics.jsonl", ha='center');
        plt.axis('off'); plt.savefig(args.out/name); plt.close()
