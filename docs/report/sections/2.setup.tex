...
\begin{equation}
  ...
  \label{eq:traj-grad}
\end{equation}
where $\boldsymbol{Z}_{i,t}$ is the token-level score contribution (e.g., advantage-weighted policy-score increment). In GRPO-style training, trajectories are sampled in \emph{prompt groups}; within a group (fixed prompt) the learning signal is common, while across groups the location may shift. Our EB development therefore treats group-specific means as nuisance parameters integrated out (Section~\ref{subsec:eb-grouped}).
