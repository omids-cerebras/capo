...
Empirically, we design an \emph{ICML-feasible} evaluation suite that can be executed within roughly one week of engineering time: a representative RLVR task (CountDown), a single open-weight backbone model (Qwen2.5--1.5B--Instruct), and two context-length regimes (2048 and 8192), and a small set of aggregation baselines that isolate the effect of aggregation under matched tokens and identical decoding/clipping (GRPO Norm, DAPO Norm, Dr.~GRPO Norm, $\Delta L$, and CAPO). We report both end performance and \emph{stability diagnostics} (monotonicity of held-out accuracy, step-to-step drift, CV/kurtosis of trajectory weights, and length-decile stratification), and we include a minimal ablation of covariance window size and shrinkage.

\paragraph{Summary of contributions.}
(i) A unified estimation-theoretic and Bayesian analysis of RLVR gradient aggregation that derives $\Delta L$ from MVUE/Jeffreys principles and clarifies invariances and bias--variance trade-offs \citep{jeffreys1946invariant,he2025deltal}; (ii) \emph{CAPO}, a covariance-aware generalization with consistent variance estimation from rollouts and closed-form plug-in weights as in \eqref{eq:capo-agg}; (iii) dependence-aware theory linking token-level autocovariance to trajectory-level variance growth, yielding principled corrections beyond length-only normalization; and (iv) an ICML-feasible empirical protocol with stability diagnostics and targeted ablations that isolate covariance-aware weighting effects under matched compute.