...
\subsection{Length-Only Benchmark and the $\Delta L$ Family}\label{subsec:warmup}
...
\begin{equation}
  ...
  v_i \propto L_i.
\end{equation}
Substituting into \eqref{eq:mvu-weights} yields $x_i\propto L_i^{-1}$, i.e., the $\Delta L$ rule with exponent $\alpha=1$ \citep{he2025deltal}. More generally, following \citet{he2025deltal} we consider the one-parameter family
\begin{equation}
...
Under the unbiasedness constraint $\sum_{i=1}^G x_i = 1/M$ and scalar trajectory variances $\{v_i\}_{i=1}^G$, the linear estimator in \eqref{eq:linear-agg} with
\begin{equation}
  x_i^\star \;=\; \frac{1}{M}\,\frac{v_i^{-1}}{\sum_{j=1}^G v_j^{-1}}
  \tag{\ref{eq:mvu-weights} revisited}
\end{equation}
achieves the minimum variance among all unbiased linear estimators.
\end{lemma}

\begin{proof}
By Lagrange multipliers on $\mathrm{Var}(\widehat{\boldsymbol{g}})$ with constraint $\sum_i x_i = 1/M$, or by Cauchy–Schwarz on the scalarized form, we obtain $x_i\propto v_i^{-1}$ and the normalization gives the stated expression.
\end{proof}

\begin{theorem}[Posterior-precision aggregator]\label{thm:ppa}
Under the model \eqref{eq:def_gi_scalar}–\eqref{eq:sigma2_posterior}, define
\begin{equation}
  \widetilde{w}_i \;:=\; \mathbb{E}\!\left[\lambda_i(\vartheta)\,\big|\,\{g_j\}\right]
  \;=\; \mathbb{E}\!\left[\frac{1}{\sigma^2}\,\omega_i(\beta,\xi)\,\Big|\,\{g_j\}\right].
  \label{eq:bayes_weight_def_repeat}
\end{equation}
Then the Bayes-optimal unbiased linear aggregate is
\begin{equation}
...
\begin{proof}
Condition on $\vartheta$; Lemma~\ref{lem:mvu} gives $x_i \propto \lambda_i(\vartheta)$. Averaging with respect to the posterior of $\vartheta$ yields \eqref{eq:bayes_weight_def_repeat} and the normalization.
\end{proof}
...
\subsection{Why $\beta<1$ When Longer Responses Are More Likely Correct}\label{subsec:alpha-less-than-1}
\paragraph{Intuition.}
...
\subsection{Weights under $k$-banded Covariance Structure}\label{subsec:kband-eta}
...
For fixed $(\rho,k,L)$ with $\rho\in(0,1)$, $s(L;\rho,k,\eta)$ is nonincreasing in $\eta$:
\[
0\le \eta_1<\eta_2 \ \Rightarrow\ s(L;\rho,k,\eta_1)\ \ge\ s(L;\rho,k,\eta_2).
\]
Consequently, the weight magnitude $[L^{\beta}s(L;\rho,k,\eta)]^{-1}$ is nondecreasing in $\eta$.
\end{lemma}
...
\subsection{Why $\beta>1$ When Longer Responses Are More Likely Correct (with tail degradation)}\label{subsec:alpha-greater-than-1}
...
\(\widehat\vartheta_G\xrightarrow{p}\vartheta^\star\) (see, e.g., van der Vaart, 1998, Theorem~5.7).
