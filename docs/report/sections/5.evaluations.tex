...
Under compound symmetry, where $v(L)$ grows superlinearly, $\Delta L$'s fixed-$\alpha$ rule under-corrects: CAPO-EB-lite (which assumes $v(L) = L^{\beta}$) shows amplification ratios rising above 1 at CS $\rho = 0.05$, whereas dependence-aware CAPO-EB achieves the lowest amplification across all long-range regimes.
...
CAPO-EB achieves the highest relative efficiency across all regimes, with gains of $2$--$6\times$ under dependence, because its inverse-variance weighting is the minimum-variance unbiased (MVU) linear combination (Lemma~\ref{lem:optimal-weights}).
...
GRPO amplifies long trajectories by $3$--$7\times$; CAPO-EB suppresses them by concentrating weight on high-precision (short) trajectories.}
...
CAPO-EB achieves $2$--$6\times$ relative efficiency under dependence, the theoretical optimum for inverse-variance weighting.}
...
Under short-range dependence, $\Delta L$ ($\alpha{=}1$) achieves the lowest ratio; under compound symmetry, CAPO-EB adapts to the superlinear variance law and dominates.}
