...
In this section we give two concrete examples illustrating the EB--CAPO algorithm:
...
\subsubsection{Simulated rollouts: CAPO--EB versus GRPO}
...
\paragraph{Compared estimators (CAPO--EB vs.\ GRPO).}
...
\begin{itemize}
  ...
  \]
  Two variants are particularly simple to implement:
  \begin{itemize}
    \item \emph{CAPO--EB-lite:} Set \(s\equiv 1\) and estimate \(\beta_t\) using Algorithm~\ref{alg:eb-lite} (no token-level ACFs required).
    \item \emph{Full CAPO--EB:} Use \(s(L;\rho,k,\eta)\) from Definition~\ref{def:kband}, update \(\beta_t\) via Corollary~\ref{cor:grad-beta}, and update \(\xi_t=(\rho_t,\eta_t)\) via Algorithm~\ref{alg:eb-capo} (possibly combined with the ACF-moment estimator, Algorithm~\ref{alg:acf-moment}).
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \textbf{CAPO branch:} Run one iteration of Algorithm~\ref{alg:eb-capo} (or EB-lite, Algorithm~\ref{alg:eb-lite}) to update \((\beta_t,\xi_t)\) and compute \((w_i^{\mathrm{CAPO}},m_{\mathrm{CAPO}}^{(t)})\).
  \end{enumerate}
...
The GRPO-family estimators use the prompt-group index to center/standardize within each group, while CAPO variants compute weights from the full batch (mirroring the reference implementation).
...
\emph{(i) GRPO normalization.}
Within each prompt group \(p\in\{1,\dots,P\}\), let \(\mathcal I_p\) denote the indices of its \(N\) trajectories.
Define
\begin{equation}
  A_i^{\mathrm{GRPO}} 
  \;:=\; 
  \frac{g_i - \bar g_p}{\widehat\sigma_p + \varepsilon},
  \qquad
  \bar g_p := \frac{1}{N}\sum_{j\in\mathcal I_p} g_j,
  \qquad
  \widehat\sigma_p^2 := \frac{1}{N}\sum_{j\in\mathcal I_p} (g_j-\bar g_p)^2,
  \label{eq:synth_grpo_adv}
\end{equation}
where \(\varepsilon>0\) is a small stabilizer.

\emph{(ii) GRPO++ / $\Delta L$ length scaling.}
For \(\alpha\in(0,\infty)\), define a normalized length factor
\begin{equation}
  r_i(\alpha)
  \;:=\;
  \frac{(L_i/\bar L_p)^{-\alpha}}{\frac{1}{N}\sum_{j\in\mathcal I_p} (L_j/\bar L_p)^{-\alpha}},
  \qquad
  \bar L_p := \frac{1}{N}\sum_{j\in\mathcal I_p} L_j,
  \label{eq:synth_grpopp_factor}
\end{equation}
and set
\begin{equation}
  A_i^{\Delta L(\alpha)} 
  \;:=\;
  r_i(\alpha)\,A_i^{\mathrm{GRPO}}.
  \label{eq:synth_grpopp_adv}
\end{equation}

\emph{(iii) CAPO--EB-lite (oracle length-only weighting).}
To isolate the effect of \emph{length-only} inverse-variance proxying, we report the oracle instantiation with exponent \(\beta=1\) and \(s\equiv 1\), i.e.,
\begin{equation}
  w_i^{\mathrm{lite}}
  \;\propto\;
  L_i^{-1},
  \qquad
  \sum_{i=1}^{B} w_i^{\mathrm{lite}}=1.
  \label{eq:synth_w_lite_oracle}
\end{equation}
Let \(m^{\mathrm{lite}}:=\sum_i w_i^{\mathrm{lite}} g_i\) and define
\begin{equation}
  A_i^{\mathrm{CAPO\text{-}EB\text{-}lite}} 
  \;:=\;
  w_i^{\mathrm{lite}}\,(g_i-m^{\mathrm{lite}}).
  \label{eq:synth_capo_eb_lite_adv}
\end{equation}

\emph{(iv) CAPO--EB (oracle dependence-aware weighting).}
To isolate dependence-aware inverse-variance reweighting, we report the oracle instantiation using the true induced variance law \(\mathrm{Var}(g_i\mid L_i)\) from \eqref{eq:synth_vL_general}:
\begin{equation}
  w_i^{\mathrm{full}}
  \;\propto\;
  \mathrm{Var}(g_i\mid L_i)^{-1}
  \;=\;
  \big(L_i\,s(L_i;\xi)\big)^{-1},
  \qquad
  \sum_{i=1}^{B} w_i^{\mathrm{full}}=1.
  \label{eq:synth_w_full_oracle}
\end{equation}
Let \(m^{\mathrm{full}}:=\sum_i w_i^{\mathrm{full}} g_i\) and set
\begin{equation}
  A_i^{\mathrm{CAPO\text{-}EB}} 
  \;:=\;
  w_i^{\mathrm{full}}\,(g_i-m^{\mathrm{full}}).
  \label{eq:synth_capo_eb_adv}
\end{equation}
In an actual training run, \(\mathrm{Var}(g_i\mid L_i)\) is replaced by the EB plug-in estimate \(\widehat v_i\) computed from rollouts (Algorithms~\ref{alg:eb-lite}--\ref{alg:eb-capo}); the oracle benchmark here is intended purely as a controlled diagnostic of the weighting rule itself.
