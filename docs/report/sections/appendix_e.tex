...
\begin{itemize}
  ...
  \begin{itemize}
    \item \emph{L--CAPO:} Set \(s\equiv 1\) and estimate \(\beta_t\) using Algorithm~\ref{alg:eb-lite} (no token-level ACFs required).
    \item \emph{Full LV--CAPO:} Use \(s(L;\rho,k,\eta)\) from Definition~\ref{def:kband}, update \(\beta_t\) via Corollary~\ref{cor:grad-beta}, and update \(\xi_t=(\rho_t,\eta_t)\) via Algorithm~\ref{alg:eb-capo} (possibly combined with the ACF-moment estimator, Algorithm~\ref{alg:acf-moment}).
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \textbf{CAPO branch:} Run one iteration of Algorithm~\ref{alg:eb-capo} (or L-CAPO, Algorithm~\ref{alg:eb-lite}) to update \((\beta_t,\xi_t)\) and compute \((w_i^{\mathrm{CAPO}},m_{\mathrm{CAPO}}^{(t)})\).
  \end{enumerate}
