...
\begin{equation}
\label{eq:capo-agg}
\bar g = \sum_{i=1}^{G} \tilde w_i g_i,
\qquad
\tilde w_i \propto \frac{L_i^{-\alpha}}{\widehat{v}_i},
\qquad
\alpha \in (0,1),
\end{equation}
where $g_i$ is a scalarized trajectory-gradient statistic (\S\ref{subsec:bayes-setup}), $L_i$ is its length, and $\widehat{v}_i$ is a plug-in estimate of the induced variance law $v(L)=\mathrm{Var}(g\mid L)$ under a chosen covariance surrogate (\S\ref{subsec:capo-plugins}). In particular, the same CAPO update can be instantiated with multiple variance-law plug-ins: \textbf{L-CAPO} (length-only/independence, including $\Delta L$), \textbf{LV-CAPO} (length\,+\,structured dependence via empirical Bayes), \textbf{CAPO-Q} (quadratic/compound-symmetry variance law), and \textbf{CAPO-HAC} (lag-window long-run variance estimation).
...
Empirically, we evaluate CAPO on a focused RLVR testbed: a representative task (CountDown), a single open-weight backbone model (Qwen2.5--1.5B--Instruct), two context-length regimes (2048 and 8192), and a compact set of aggregation baselines that isolate the effect of aggregation under matched tokens and identical decoding/clipping (GRPO Norm, DAPO Norm, Dr.~GRPO Norm, $\Delta L$, and CAPO). In addition to end performance, we report \emph{stability diagnostics} (monotonicity of held-out accuracy, step-to-step drift, CV/kurtosis of trajectory weights, and length-decile stratification), together with a targeted ablation over the covariance bandwidth used by CAPO's empirical-Bayes estimator. Finally, to isolate the advantage transformation in a setting where the length--variance law is known by construction, we include a base-model-free synthetic covariance benchmark that compares CAPO, GRPO, and $\Delta L$ under controlled dependence regimes (\S\ref{subsec:synthetic-benchmark} and Appendix~\ref{sec:synthetic-covariance-benchmark}).

\paragraph{Summary of contributions.}
(i) A unified estimation-theoretic and Bayesian analysis of RLVR gradient aggregation that derives $\Delta L$ from MVUE/Jeffreys principles and clarifies invariances and bias--variance trade-offs \citep{jeffreys1946invariant,he2025deltal}; (ii) \emph{CAPO}, a covariance-aware generalization with closed-form inverse-variance weights and rigorous robustness guarantees for plug-in variance estimation; (iii) a modular \emph{variance-law plug-in} perspective that yields multiple submission-ready instantiations---\textbf{L-CAPO} (length-only/independence), \textbf{LV-CAPO} (empirical Bayes with structured dependence), \textbf{CAPO-Q} (quadratic/compound symmetry), and \textbf{CAPO-HAC} (lag-window long-run variance); (iv) dependence-aware theory linking token-level autocovariance to trajectory-level variance growth, yielding principled corrections beyond length-only normalization; (v) an empirical protocol with stability diagnostics and targeted ablations that isolate covariance-aware weighting effects under matched compute; and (vi) a controlled synthetic covariance benchmark that compares GRPO, $\Delta L$, and CAPO advantage constructions under known dependence regimes, making length amplification and signal allocation effects explicit.
